...
const Modal = ({ showModal }) => {
    ...
    return (
        <div 
        className={`w-3/4 mx-auto rounded-md bg-white p-10 mt-8 flex flex-col overflow-y-scroll mod`} onMouseDown={handleModalClick}>
            <h6 className='text-2xl font-semibold'>Latest Trends</h6>
            <div className='flex items-start justify-between w-full gap-6 mt-6'>
                {latestTrends.map((product, index) => (
                    <div className='w-1/5 cursor-pointer' key={index}>
